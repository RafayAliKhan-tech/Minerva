...
#             # --------------------------------------------------------
#     # Rebuild competition ranks from deterministic percentages
#     # --------------------------------------------------------
...
#         return (
#             f"{career_name} received the highest deterministic "
#             f"score of {score_text}. The result is supported by "
#             f"signals such as {dimensions_text}. This area "
...
#     return (
#         f"{career_name} received the highest deterministic score "
#         f"of {score_text}. This makes it a useful area to explore "
...
def normalize_career_scores(
    ...
    for index, item in enumerate(
        ...
        # The deterministic scoring engine stores the career ID in
        # "career" and the display name in "career_name", so
        # "career_name" must be read first. Reading "career" first
        # leaked the raw id ("data") into every user-facing string
        # instead of the display name ("Data & Analytics").
    ...
    # --------------------------------------------------------
    # IMPORTANT FIX (v5.1):
    # Return the RANK-SORTED list, not the original input-order
    # list. Downstream consumers (build_primary_career,
    # build_alternative_careers) assume career_scores[0] is the
    # highest-ranked career and career_scores[1:3] are the next
    # two highest. Previously this function computed correct
    # rank values but returned them in the original, unsorted
    # order, which silently picked the wrong "primary" career
    # whenever the top scorer was not first in the source file.
    # --------------------------------------------------------
...
def extract_dimensions_from_question_results(
    career_id: str,
    deterministic_result: Dict[str, Any]
) -> List[str]:
    """
    Derive the dimensions that belong to one career from
    question_results, which is where the deterministic engine
    records the career -> dimension link.

    Only dimensions the user actually answered correctly are
    returned, so nothing unsupported is claimed as support.
    Ordering follows the engine's own dimension percentages so the
    strongest supporting signal is listed first.
    """

    question_results = deterministic_result.get(
        "question_results",
        []
    )
    ...
    # Dimension percentages, used for ordering only.

    strength: Dict[str, float] = {}

    dimension_rows = deterministic_result.get(
        "dimensions",
        []
    )
    ...
    for item in question_results:
        ...
        item_career = normalize_text(
            item.get("career")
            or item.get("career_id")
        )

        if item_career != career_id:
            continue
...
def extract_dimensions_for_career(
    ...
    # --------------------------------------------------------
    # The deterministic scoring engine emits "dimensions" as a
    # FLAT LIST of aggregates carrying no career_id, so the
    # dict-with-"primary" shape below never matched and this
    # function always returned [] - which silently collapsed every
    # career's supporting_dimensions onto the same global AI
    # strengths. The career -> dimension link does exist in
    # question_results, so derive it from there for the list shape
    # and keep the dict shape for backward compatibility.
    # --------------------------------------------------------
    ...
    if not isinstance(
        ...
        return extract_dimensions_from_question_results(
            career_id,
            deterministic_result
        )
...
def build_why_it_fits(
    ...
    is_primary:
        True  - this career is the highest deterministic result.
        False - this career is an alternative, so it must never be
                described as the highest scorer.
    """
    ...
    if is_primary:
        ...
        lead = (
            f"{career_name} scored {score_text}, placing it below "
            f"your highest-scoring area while still being worth "
            f"exploring."
        )
    ...
    if supporting_dimensions:
        ...
        return (
            f"{lead} The result is supported by "
            f"signals such as {dimensions_text}. This area "
            f"involves {focus} and is a useful direction to "
            "explore further."
        )
    ...
    return (
        f"{lead} This makes it a useful area to explore "
        "further through practical learning and small projects."
    )
