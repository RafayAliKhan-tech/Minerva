...
def _target_careers(
    ...
    Journey 1 evaluates every supported career. Recommendation metadata
    identifies the leading career for presentation, but it must not limit
    roadmap-ready skill profiles to that career.
    """
    ...
    if rec_type == "highest_score_tie":
        ...
        if isinstance(
            ...
            if isinstance(tied, list):
                ...
                for value in tied:
                    career = _normalize_id(value)
                    if career in CAREERS and career not in careers:
                        careers.append(career)

                if careers:
                    # A user may generate a roadmap for any scored career,
                    # not only the highest-score tie. Keep every supported
                    # career in the skill profile so lower-scoring (including
                    # 50% and 0%) selections have a valid roadmap input.
                    return list(CAREERS)
