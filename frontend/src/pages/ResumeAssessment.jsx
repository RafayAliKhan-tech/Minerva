...
function ResumeAssessment() {
  ...
  const activities = getResumeActivities(profile.id)
  ...
  useEffect(() => {
    if (!activity) return
    const savedResponses = JSON.parse(sessionStorage.getItem('resumeResponses') || '{}')
    const saved = savedResponses[activity.id] || {}
  ...
  const saveResponse = (auto = false) => {
    ...
    const responses = JSON.parse(sessionStorage.getItem('resumeResponses') || '{}')
  ...
  const handleNext = () => {
    ...
    if (activityIndex < activities.length - 1) {
      navigate(`/explore/resume/assessment/${activityIndex + 2}`)
    } else {
  ...
  const handlePrevious = () => {
    if (activityIndex > 0) {
      navigate(`/explore/resume/assessment/${activityIndex}`)
    } else {
  ...
  if (!resumeFile) {
    return (
      <AssessmentLayout onBack={() => navigate('/explore/resume')} showProgress={false}>
        <p className="text-center text-brown-light">Upload a resume first to continue.</p>
      </AssessmentLayout>
    )
  }
  ...
  if (!activity) {
    return (
      <AssessmentLayout onBack={() => navigate('/explore/resume/insights')} showProgress={false}>
        <p className="text-center text-brown-light">No assessment activities available right now.</p>
      </AssessmentLayout>
    )
  }
  ...
  return (
    <AssessmentLayout
      ...
      subtitle={`${profile.title}: ${activity.description}`}
    >
      ...
      <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button onClick={handleNext} variant="dark" size="md" disabled={!canContinue}>
          {activityIndex === activities.length - 1 ? 'See Results' : 'Continue'}
        </Button>
      </div>
    </AssessmentLayout>
  )
