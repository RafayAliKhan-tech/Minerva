...
const AUTH_PAGES_WITH_FOOTER = ['/login', '/signup', '/forgot-password']
...
function Layout({ showFooter = true }) {
  ...
  const shouldShowFooter = showFooter && !isChatPage && (!isAuthPage || AUTH_PAGES_WITH_FOOTER.includes(pathname))
  ...
  return (
    <div className={`page-shell${isAuthPage ? ' auth-page-shell' : ''}${isChatPage ? ' chat-app-shell' : ''}`}>
      ...
      {shouldShowFooter && <Footer compact={isAuthPage} />}
